...
- **Logo**: derived from `meta.siteName` the same way `Hero.astro` derives its `<h1>` (split on `" with "`), rendered via `SpritzTitle` at a small size — but styled as a fully cursive/italic wordmark (`font-style: italic` on `--font-heading`), distinct from Hero's bold uppercase title treatment. This is why `Navbar.astro` now declares a real `Props { meta: SiteMeta }` — it previously declared no props at all and silently ignored the `meta` prop `Layout.astro` was already passing it.
...
- `<h1>` is `text-transform: uppercase` now; the second line ("with Lara", derived from `meta.siteName` the same way as before) opts back out via `text-transform: none` plus `font-style: italic` on `--font-heading`, so it reads as a cursive script contrasting the bold caps above it.
...
- The portrait photo is a new `PlaceholderImage` variant, `"square"` (`aspect-ratio: 1/1`, always cropped — added alongside `circle`/`rounded`/`portrait`), then faded on one edge via `mask-image: linear-gradient(to left, black 55%, transparent 100%)` on `.hero-portrait :global(.placeholder-image)` so the image blends into the hero's teal background rather than having a hard edge. The mask is dropped on mobile (`max-width: 800px`) since the layout stacks the photo below the copy instead of beside it. `hero.image`'s placeholder default now points at `/images/lara_about.jpg` (same photo as `About.astro`, just as a stand-in until a dedicated hero photo is uploaded) — still a normal admin-editable field, no code change needed to swap it.
- `content.socialLinks` (still just `[instagram, email]`) moved from an inline row under the CTA to two `position: absolute` circular buttons pinned top-right of the section, offset below the fixed navbar via `top: calc(var(--navbar-height) + ...)`.
- The CTA button's copy ("Let's learn together →") is hardcoded in the component, same as the previous "Book a first lesson" — it was never a `siteContent.json` field either time, so this isn't a new gap.
